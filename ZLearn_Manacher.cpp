...
vector<string> Manacher(const string & strSrc)
{
    ...
    for (int iIndex = 0; iIndex < vecPalindromicRadius.size(); iIndex ++)
    {
...
public:
    ...
    {
        vector<string> vecPalindromicSubstring = Manacher(s);
        
        int iResultIndex = -1;
        
        for (int iIndex = 0; iIndex < vecPalindromicSubstring.size(); iIndex ++)
        {
            if (vecPalindromicSubstring[iResultIndex].length() < vecPalindromicSubstring[iIndex].length())
            {
                iResultIndex = iIndex;
            }
        }
        
        return iResultIndex < 0 ? "" : vecPalindromicSubstring[iResultIndex];
    }
    
private:

    static const char C_UNIT_SEPARATOR = '\x1F';
